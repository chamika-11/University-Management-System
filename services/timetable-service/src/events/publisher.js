...
const publish = async (topic, { eventType, payload }) => {
  ...
  try {
    await getProducer().send({
      topic,
      messages: [{
        key: payload.sectionId || payload.studentId || uuidv4(),
        value: JSON.stringify({ eventType, payload, meta: { eventId: uuidv4(), timestamp: new Date().toISOString(), source: 'timetable-service', version: '1.0' } }),
      }],
    });
